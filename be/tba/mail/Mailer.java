...
public class Mailer
{
	...
	static public boolean sendMail(WebSession webSession, int accountId, String subject, String body)
	{
		...
		{
			Date date = new Date();
			Address[] vTo = prepMailAddrArr(vEmailAddresses);
	...
	static public boolean sendCallInfoMail(WebSession webSession, int accountId, boolean isLogOn)
	{
		...
		Collection<CallRecordEntityData> vRecords = vQuerySession.getDocumentedNotMailed(webSession, vCustomer.getId(), false); //isLogOn);
		...
		if (!vRecords.isEmpty() || !vCustomer.getNoEmptyMails())
		{
			...
			if (sendMail(webSession, accountId, "Uw oproepenlijst tot " + DateFormat.getDateInstance(DateFormat.LONG, new Locale("nl", "BE")).format(new Date()) + " " + vCustomer.getFullName(), vBody.toString()))
			{
			...
			{
				log.error("sendCallInfoMail ended failed for: " + vCustomer.getFullName());

			}
		}
		else
		{
			log.info("sendCallInfoMail ended. Mail not send: #records=" + vRecords.size() + ", noEmptyMailsFlag=" + vCustomer.getNoEmptyMails());
		}
		return ret;
	}
	...
	static public boolean mailInvoice(InvoiceEntityData invoiceData)
	{
		...
		try
		{
			...
			Date date = new Date();
			vTo = prepMailAddrArr(vCustomer.getInvoiceEmail());
			if (System.getenv("TBA_MAIL_ON") == null)
			{
				// development path
				vTo = new InternetAddress[1];
	...
	static private StringBuilder buildMailBody(AccountEntityData account, Collection<CallRecordEntityData> records, AtomicBoolean isImportant)
	{
		...
		else
		{
			...
			if (records.size() == 1) vBody.append("<br>Er is </span><span class=\"bodyredbold\">1</span><span class=\"bodytekst\"> nieuwe oproep beschikbaar.");
			else vBody.append("<br>Er zijn </span><span class=\"bodyredbold\">" + records.size() + "</span><span class=\"bodytekst\"> nieuwe oproepen beschikbaar.");
			// int vNewCnt = 0;
			...
			for (Iterator<CallRecordEntityData> i = records.iterator(); i.hasNext();)
			{
				...
				if (vEntry.getIsIncomingCall()) vInOut = vInOut.concat("images/incall.gif");
				else vInOut = vInOut.concat("images/outcall.gif");
				...
				if (vLongDesc.length() != 0) vBody.append("<img src=\"" + Constants.TBA_URL_BASE + "images/info.gif\" alt=\"Extra info\" height=\"16\" border=\"0\">");
				if (vEntry.getIsAgendaCall()) vBody.append("<img src=\"" + Constants.TBA_URL_BASE + "images/agenda.gif\" height=\"13\" border=\"0\">&nbsp;");
				if (vEntry.getIsSmsCall()) vBody.append("<img src=\"" + Constants.TBA_URL_BASE + "images/sms.gif\"  height=\"13\" border=\"0\">&nbsp");
				if (vEntry.getIsForwardCall()) vBody.append("<img src=\"" + Constants.TBA_URL_BASE + "images/telefoon.gif\"  height=\"13\" border=\"0\">&nbsp;");
				if (vEntry.getIsFaxCall()) vBody.append("<img src=\"" + Constants.TBA_URL_BASE + "images/fax.gif\"  height=\"13\" border=\"0\">&nbsp;");
				vBody.append("</td></tr>");
				// vRowInd++;
			}
			vBody.append("</table>");
		...
		vBody.append(
		      "U kan het tijdstip van deze mail zelf instellen als u zich aanmeldt op onze webpagina (<a href=\"http://www.theBusinessAssistant.be\">www.theBusinessAssistant.be</a>).<br>");
...
//      vBody.append("Eventuele extra informatie aangegeven met het <img src=\"" + Constants.TBA_URL_BASE + "images/info.gif\" alt=\"Extra info\" height=\"16\" border=\"0\"> &nbsp;icoontje, kan daar ook geraadpleegd worden.<br>");
	...
	static private StringBuilder buildXmlBody(AccountEntityData account, Collection<CallRecordEntityData> records, AtomicBoolean isImportant)
	{
		...
		for (Iterator<CallRecordEntityData> i = records.iterator(); i.hasNext();)
		{
			...
			if (vLongDesc.length() > 0) vBody.append("  <info>" + vLongDesc + "</info>\n");
			if (vEntry.getIsAgendaCall()) vBody.append("  <meeting>true</meeting>\n");
			if (vEntry.getIsSmsCall()) vBody.append("  <sms>true</sms>\n");
			if (vEntry.getIsForwardCall()) vBody.append("  <forward>true</forward>\n");
			if (vEntry.getIsImportantCall()) vBody.append("  <belangrijk>true</belangrijk>\n");
			if (vEntry.getIsFaxCall()) vBody.append("  <fax>true</fax>\n");
			vBody.append(" </object>\n");
			if (vEntry.getIsImportantCall())
			{
				isImportant.set(true);
	...
	static private Address[] prepMailAddrArr(String mailAddrStr) throws MessagingException
	{
		mailAddrStr = mailAddrStr.replace(',', ';');
		if (mailAddrStr.endsWith(";"))
		{
		...
		Address[] toArr = new InternetAddress[vMailTokens.countTokens()];
		int i = 0;
		while (vMailTokens.hasMoreTokens())
		{
			...
			{
				addr = addr.trim();
				toArr[i++] = new InternetAddress(addr);
			}
		}
		return toArr;
	}
