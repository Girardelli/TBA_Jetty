...
public class InvoiceHelper
{
   ...
   public void storeOrUpdate(WebSession webSession)
   {
      ...
      if (mInvoiceEntityData == null)
      {
         ...
         if (mInvoiceEntityData != null)
         {
            ...
            if (mInvoiceEntityData.getCreditId() >= 0)
            {
               ...
               // log.info(mAccountEntityData.getFwdNumber() + ": Last invoice was a credit
               // note: make new on for this months");
            ...
            else if (mInvoiceEntityData.getFrozenFlag())
            {
               ...
               mInvoiceEntityData = null;
               // log.info(mAccountEntityData.getFwdNumber() + " : Last invoice was frozen:
               // make a new one");
            }
            else
            {
               ...
               // figures till now.
               vStart = mInvoiceEntityData.getStartTime();
               // log.info(mAccountEntityData.getFwdNumber() + ": Last invoice was not frozen:
               // update this one");
            }
         ...
         {
            // log.info(mAccountEntityData.getFwdNumber() + " : no invoices yet for this
            // number for this month. Make one.");
      ...
      else
      {
         ...
         if (mInvoiceEntityData.getCreditId() >= 0)
         {
            // log.info(mAccountEntityData.getFwdNumber() + ": credit note or original
            // invoice was provided: show the frozen data.\r\nnew Start: " + vStart + " ==>
            // " + mInvoiceEntityData.getStartTime() + "\r\nnew Stop: " + vEnd + " ==> " +
            // mInvoiceEntityData.getStopTime());
            vStart = mInvoiceEntityData.getStartTime();
         ...
         else if (mInvoiceEntityData.getFrozenFlag())
         {
            ...
            // starting from the stopTime of this last one.
            // log.info(mAccountEntityData.getFwdNumber() + " : frozen invoice data, not
            // related to credit note, provided by caller: show the frozen data.\r\nnew
            // Start: " + vStart + " ==> " + mInvoiceEntityData.getStartTime() + "\r\nnew
            // Stop: " + vEnd + " ==> " + mInvoiceEntityData.getStopTime());
            vStart = mInvoiceEntityData.getStartTime();
            vEnd = mInvoiceEntityData.getStopTime();
         }
         else
         {
            // the last invoice is not frozen: update this invoice with the
            // figures till now.
            // log.info(mAccountEntityData.getFwdNumber() + " : NOT frozen invoice data
            // provided by caller: update this one.\r\nnew Start: " + vStart + " ==> " +
            // mInvoiceEntityData.getStartTime());
            vStart = mInvoiceEntityData.getStartTime();
         }
      }
      ...
      else
      {
         ...
         {
            // log.info("new total cost " + mInvoiceData.TotalCost + " is not equal to old
            // cost " + mInvoiceEntityData.getTotalCost());
            mInvoiceEntityData.setTotalCost(mInvoiceData.TotalCost);
            // log.info("invoice not frozen: update start-stop: start: " +
            // mInvoiceEntityData.getStartTime() + "-->" + vStart + ", stop: " +
            // mInvoiceEntityData.getStopTime() + "-->" + mCallCounts.LastInvoiceItem);
            mInvoiceEntityData.setStartTime(vStart);
            mInvoiceEntityData.setStopTime(mCallCounts.LastInvoiceItem);
   ...
   public boolean generatePdfInvoice()
   {
      ...
      {
         // log.info("No invoice doc generated for " + mAccountEntityData.getFullName() +
         // ": getNoInvoice()=" + mAccountEntityData.getNoInvoice() + ",
         // getFrozenFlag()=" + mInvoiceEntityData.getFrozenFlag());
         return true;
      }
      if (mInvoiceEntityData == null)
      {
         // log.info("No invoice data found in DB for " +
         // mAccountEntityData.getFullName());
         return true;
      }
   ...
   public int setCounters(CallCounts callCounts, Collection<CallRecordEntityData> callList, AccountEntityData customerData)
   {
      if (callList != null)
      {
         ...
         callCounts.TotalCalls = callCounts.InCalls + callCounts.OutCalls;// +
                                                                          ...
                                                                          // mFaxCalls;
         // if (customerData != null)
         // log.info("setCounters for " + customerData.getFullName() + ": " +
         // callList.size() + "(in), " + callCounts.TotalCalls + "(total); " +
         // callCounts.LongFwdCallSec + "sec (" + callCounts.LongFwdCalls + " calls)");
         // else
         // log.info("setCounters for unknown: " + callList.size() + "(in), " +
         // callCounts.TotalCalls + "(total); " + callCounts.LongFwdCallSec + "sec (" +
         // callCounts.LongFwdCalls + " calls)");
      }
