...
public class CallRecordFacade
{
   ...
   /*
    * This method is called when a new call is created from scratch. An incoming
    * call shall (can be) be selected later when the call is ended and recorded by
   ...
   // because this is a server loop call, possible changes to that new call data
   // must be stored
   ...
   public static void updateNewUnmappedCall(SessionParmsInf parms, WebSession webSession)
   {
      ...
      {
         // strange situation
         // System.out.println("updateNewUnmappedCall called with a valid
         // NewUnmappedCall");
         // make a new one.
   ...
   public static boolean saveNewCall(SessionParmsInf parms, WebSession webSession)
   {
      ...
      updateNewUnmappedCall(parms, webSession);
      ...
      if (vNewRecord != null)
      {
         ...
         {
            // System.out.println("there are subcustomers. Set the super customer=" +
            // vNewRecord.getFwdNr() + ", record key=" + vKey);
            // set the fwdNr of the super customer so that selectSubCustomer.jsp can prepare
            // the sub customers list
            // parms.setAttribute(Constants.ACCOUNT_ID, vNewCall.getId());
